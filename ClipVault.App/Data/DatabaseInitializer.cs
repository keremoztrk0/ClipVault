...
public class DatabaseInitializer(ILogger<DatabaseInitializer> logger)
{
    ...
    /// </summary>
    public async Task InitializeAsync(IDbConnectionFactory context)
    {
        await using SqliteConnection connection = await context.GetOpenConnectionAsync();
    ...
    private async Task MigrateAppSettingsAsync(SqliteConnection connection)
    {
        try
        {
            ...
            if (!columnList.Contains("RetentionDays"))
            {
                ...
                logger.LogInformation("Added RetentionDays column to AppSettings");
            }
            ...
            if (!columnList.Contains("StartMinimized"))
            {
                ...
                logger.LogInformation("Added StartMinimized column to AppSettings");
            }
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Migration check failed, columns may already exist");
        }
    }
    ...
    private static async Task SeedDataAsync(SqliteConnection connection)
    {
        ...
        if (settingsExist == 0)
        {
            await connection.ExecuteAsync("""
                                                          ...
                                                          VALUES (1, 'Ctrl+Shift+V', 'Dark', 1000, 0, 0, 0, 1)
                                          """);
        }
    }
}
